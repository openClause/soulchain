...
SoulChain is built as a first-class [OpenClaw](https://github.com/openClause/openclaw) skill. Install it once and your agent gets sovereign identity automatically.
...
The agent can then use SoulChain through the skill's scripts — init, status, verify, history, restore — all documented in `SKILL.md` and picked up automatically.

### Programmatic

```typescript
import { activate, deactivate, getEngine } from '@openclaused/soulchain/openclaw';

// Start — installs filesystem hooks + file watcher
...
When activated, every write to a tracked file is automatically encrypted, signed, and anchored. No manual steps. The agent doesn't need to know or care — its identity is being protected in the background.
