...
async function initSoulchain(workspaceDir, options) {
    ...
    // 1. Generate keypair
    const keypair = (0, core_1.generateKeypair)();
    const pubHex = (0, core_1.toHex)(keypair.publicKey);
    const address = (0, core_1.publicKeyToAddress)(keypair.publicKey);
    // 2. Create keystore
    ...
    const keystoreData = await (0, core_1.createKeystore)(keypair.secretKey, passphrase);
    ...
    const storage = (0, core_1.createStorageAdapter)(config);
    const cryptoProvider = (0, crypto_provider_1.createCryptoProvider)(keypair);
    const engine = new core_1.SyncEngine(config, cryptoProvider, storage, chain);
